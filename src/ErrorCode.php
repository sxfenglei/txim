...
class ErrorCode {
    ...
    public static $errCode = [
        ...
        6206 => 'UserSig 过期，请重新获取有效的 UserSig 后再重新登录，获取方法请参见 生成 UserSig。',
        ...
        -10001 => 'D2 过期。D2 是根据 UserSig 生成的内部票据，D2 的有效期小于或等于 UserSig 的有效期。请重新调用 TIMManager.getInstance().login 登录接口生成新的 D2。',
        -10003 => 'A2 过期。A2 是根据 UserSig 生成的内部票据，A2 的有效期小于或等于 UserSig 的有效期。请重新调用 TIMManager.getInstance().login 登录接口生成新的 A2。',
        -10004 => '处理下行包时发现 A2 验证没通过或者被安全打击。请重新调用 TIMManager.getInstance().login 登录接口生成新的 A2。',
        -10005 => '不允许空 D2Key 加密。',
        ...
        70003 => 'UserSig 非法，请使用官网提供的 API 重新 生成 UserSig。',
        70005 => 'UserSig 非法，请使用官网提供的 API 重新 生成 UserSig。',
        70009 => 'UserSig 验证失败，可能因为生成 UserSig 时混用了其他 SDKAppID 的私钥或密钥导致，请使用对应 SDKAppID 下的私钥或密钥重新 生成 UserSig。',
        70013 => '请求中的 UserID 与生成 UserSig 时使用的 UserID 不匹配，您可以在即时通信 IM 控制台的【 开发辅助工具】页面校验 UserSig。',
        70014 => '请求中的 SDKAppID 与生成 UserSig 时使用的 SDKAppID 不匹配，您可以在即时通信 IM 控制台的【 开发辅助工具】页面校验 UserSig。',
        70016 => '公钥不存在，UserSig 验证失败，请在即时通信 IM 控制台 获取密钥。',
        70020 => 'SDKAppID 未找到，请在即时通信 IM 控制台确认应用信息。',
        70050 => 'UserSig 验证次数过于频繁。请检查 UserSig 是否正确，并于1分钟后重新验证。您可以在即时通信 IM 控制台的【 开发辅助工具】页面校验 UserSig。',
        70051 => '帐号被拉入黑名单。',
        70107 => '请求的用户帐号不存在。',
        ...
        20007 => '发送单聊消息，被对方拉黑，禁止发送。消息发送状态默认展示为失败，您可以登录控制台修改该场景下的消息发送状态展示结果，具体操作请参见 黑名单检查。',
        ...
        90043 => 'JSON 格式请求包中 OfflinePushInfo 不符合消息格式描述，请参考 OfflinePushInfo 对象 的定义。',
    ...
    //返回中文错误信息
    public static function getCN($arr){
        if($arr['ErrorCode']){
            $arr['ErrorInfo'] = isset(self::$errCode[$arr['ErrorCode']])?self::$errCode[$arr['ErrorCode']]:$arr['ErrorInfo'];
        }
        return $arr;
    }
